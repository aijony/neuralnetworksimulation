...
public class Propagation {
	...
	/*
	 * Forward propagates by multiplying the weights by previous neurons and activates
	 * Loops through the entire network to propagate	
	*/
	public void forward(double[] input) {
		...
		for(currentLayer = 1; currentLayer < neurons.size(); currentLayer++){
			//Calculates neurons based off of the previous neurons and the weights
	...
	*/
	...
	/*
	 * Back propagation method for matrix math:
	 * Runs through calculating semi-persistant deltas and 
	 * using them to calculate proceeding deltas and new weights
	*/
	public void backward(double [] ideal) {
		...
		//Initializes array list to a certain sides since the back propagation will be working backwards
		...
		//Starts at the input and works to the last hidden layer setting a delta matrix that aligns with a neuron matrix (1D shaped 2D arrays)
	...
	/*
	 * Calculates the delta either based off of the ideal and actual if it is the first one
	 * or the sum of the previous deltas times the weight. Then multiplies that value by the sigmoid prime
	 * of the proceeding neurons net input
	*/
	private double[][] calculateDelta(int synapseLayer) {
		...
		if(synapseLayer == connections.size() -1){
			...
			//(This also deals with the summation due to the properties of matrix multiplication)
	...
	/*
	 * Calculates the new weight based off of the derivative of the cost, learning rate, and old weight
	*/
	private double[][] calculateWeight(int synapseLayer) {
		 ...
		 //Multiplies the delta by the neurons  value (Calculated in forward propagation and activating it)
