...
public class MatrixMath {
	...
	/*
	 * Matrix multiples 2 matrices, operator X size must be equal to operand Y
	 * size Please have a understanding of matrix multiplication before using
	 * this code
	 */
		...
		for (int operatorRow = 0; operatorRow < operatorRowAmnt; operatorRow++) {
			for (int operandColumn = 0; operandColumn < operandColumnAmnt; operandColumn++) {
				for (int operateIndex = 0; operateIndex < definedDimensionSize; operateIndex++) {
					product[operandColumn][operatorRow] = product[operandColumn][operatorRow]
							+ (operator[operateIndex][operatorRow] * operand[operandColumn][operateIndex]);
				}
			}
		}
		return product;
	...
	/*
	 * Subtracts all values from one matrix to another The matrices must be
	 * identical in size
	 */
	...
	// Converts a 1D array into a 2D array in the inverse dimension of row to
	// matrix
